...
--
-- Setup:
--   1. SQL editor -> paste this query -> set the `catalog` / `schema` named parameters
...
--
-- The table name is assembled from the :catalog / :schema parameters via IDENTIFIER(),
-- which is how Databricks SQL parameterizes an object name (a plain {{param}} only
-- substitutes into a value position, not a table reference).
--
...
FROM IDENTIFIER(:catalog || '.' || :schema || '.caught_in_air_inference_profile_metrics') AS p
WHERE p.column_name = ':table'          -- table-level rows carry the model-quality metrics
  ...
  AND p.window.start = (
    ...
    FROM IDENTIFIER(:catalog || '.' || :schema || '.caught_in_air_inference_profile_metrics') AS p2
...
-- FROM IDENTIFIER(:catalog || '.' || :schema || '.caught_in_air_inference_drift_metrics') AS d
-- WHERE d.column_name = 'predicted_caught'
--   AND d.drift_type = 'CONSECUTIVE'
--   AND d.window.start = (
--     SELECT max(d2.window.start)
--     FROM IDENTIFIER(:catalog || '.' || :schema || '.caught_in_air_inference_drift_metrics') AS d2
