...
--
-- Setup:
--   1. Replace <catalog>.<schema> below with the catalog and schema where the monitor's
--      metric tables live (both occurrences, e.g. main.dqm).
--   2. SQL editor -> paste the query -> run to confirm it returns a row.
--   3. Save the query, then create an Alert on it:
--        Column:    latest_accuracy
--        Condition: is below
--        Threshold: 0.85
--   4. Schedule it (e.g. daily) and add a destination (email / Slack / webhook).
--
